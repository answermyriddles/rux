function opt_c = doses_simulation_sens(b_sens, d_sens)
close all
% clear
...
jj= 1; % simulate 1 week
    ...
    for ii = 1:length(doses)
        ...
        [InitB, InitD]  = bdrates_sens(InitConc, b_sens, d_sens);
        [boff, doff]    = bdrates_sens(0, b_sens, d_sens); 
        ...
        [InitB2,InitD2] = bdrates_sens(conc(2), b_sens, d_sens); 
        cpop(2)         = cpop(1).*exp((InitB2-InitD2).*PK_cycle);
            ...
            for i = 3:numCycles  
                ...
                [bon, don]  = bdrates_sens(conc(i), b_sens, d_sens);  
...
ylabel('Simulated JAK2V617F cell population (Log_2)','FontSize',14,'fontWeight','bold')
...
%% to remove .mat file tag
bsens_title  = strrep(b_sens, '_', ' ')
dsens_title  = strrep(d_sens, '_', ' ')

title({bsens_title ; dsens_title}) 
% set(gca,'yscale','log')
fileName = ['cellPopulationsinaWeekFiveDoses1Week_', b_sens, '_', d_sens];
